...
def saveMLResults(all_yearsDf, final_yearDf, xTest, yTest, N, xDf, yDf, modelList, workingDir, numFeatures, printResults=True):
    """
    inputs: N - int - the number of times the model should be trained and evaluated.
            xDf - pandas dataframe - the rows represent the data points, the columns represent the features. These
                                         are the inputs into the model
...
#         joblib.dump((bestModel, bestFeatures, bestMAE, bestRSq, avgMAE, avgRSq), saveLoc)
        ...
        m, r = actualTest(all_yearsDf, final_yearDf, xTest, yTest, yDf, bestModel)
...
def actualTest(all_yearsDf, final_yearDf, xTest, yTest, yDf, model):
    ...
    for i in range(len(pred)):
        pred[i] = final_yearDf['yield'][i] + pred[i]
    print(pred)
...
#     p1 = max(max(pred), max(yTest['yield']))
#     p2 = min(min(pred), min(yTest['yield']))
#     plt.plot([p1, p2], [p1, p2], color='k', label='Perfect Predictions')
#     plt.xlabel('Predicted Yields')
#     plt.ylabel('True Yields')
#     plt.legend()
    ...
    # find the R values
    r = np.corrcoef(yTest['yield'], pred)[0,1]
...
#     r2y = yTest.tolist()
    ...
    yTestYield = yTest['yield']
    result = sm.OLS(yTestYield, r2pred).fit()

    # printing the summary table
    print(result.summary())
    
    #accuracy = accuracy_score(yTest, pred)
    print('skt mape: ' + str(round(skt_mape(yTestYield, pred) * 100, 3)) + '%')
    print('skt smape: ' + str(round(skt_smape(yTestYield, pred) * 100, 3)) + '%')
    print('skt mae: ' + str(round(skt_mae(yTestYield, pred), 3)))
    print('skt mse: ' + str(round(skt_mse(yTestYield, pred), 3)))
    print('skt rmse: ' + str(round(math.sqrt(skt_mse(yTestYield, pred)), 3)) + '\n\n')
