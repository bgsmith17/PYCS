...
def getBestModel(N, xDf, yDf, emptyModel, paramGrid, features, metricToOptimize='r2'):
    ...
    #for train_index, test_index in zip(X[:224], X[224:]):
        ...
        xgb_accuracy = accuracy_score(yTest, pred)
    ...
    ## return the results
    return avgMAE, avgRSq, bestMAE, bestRSq, bestModel, featuresUsed, accuracy, f1, mcc, xgb_model, xgb_accuracy

# def saveMLResults(xBoost, yBoost, xTest, yTest, N, xDf, yDf, modelList, workingDir, numFeatures, printResults=True):
def saveMLResults(xTest, yTest, N, xDf, yDf, modelList, workingDir, numFeatures, printResults=True):
    ...
    for tup in modelList:
        ...
        # get results
        avgMAE, avgRSq, bestMAE, bestRSq, bestModel, bestFeatures, accuracy, f1, mcc, xgb_model, xgb_training_accuracy = getBestModel(N, xDf, yDf, model, paramGrid, 
...
#         xgb_accuracy = actualTest(xBoost, yBoost, xTest, yTest, xgb_model, bestModel.best_params_)
        xgb_accuracy = actualTest(xTest, yTest, xgb_model, bestModel.best_params_)
        #fig=plot_confusion_matrix(bestModel, xTest, yTest)
        fig = plot_confusion_matrix(xgb_model, xTest, yTest)
...
#         print("R sq: ", r)
        ...
        print("xgb training accuracy: ", xgb_training_accuracy)
        print("accuracy score for PYCS: ", xgb_accuracy)
            
        #count += 1
    #return bestModel
        
# def actualTest(xBoost, yBoost, xTest, yTest, model, params):
def actualTest(xTest, yTest, model, params):
    
    print("test results on our test data: ")
    
    #model = model.fit(xBoost, yBoost, xgb_model=model.get_booster())
    # get predictions
    pred = model.predict(xTest)
    #trainPred = model.predict(xTrain)
