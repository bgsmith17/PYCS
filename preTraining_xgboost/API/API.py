...
def saveMLResults(boost_xDf, boost_yDf, xTest, yTest, N, xDf, yDf, modelList, workingDir, numFeatures, printResults=True):
    """
    inputs: N - int - the number of times the model should be trained and evaluated.
            xDf - pandas dataframe - the rows represent the data points, the columns represent the features. These
                                         are the inputs into the model
...
#         joblib.dump((bestModel, bestFeatures, bestMAE, bestRSq, avgMAE, avgRSq), saveLoc)
        ...
        m, r = actualTest(boost_xDf, boost_yDf, xTest, yTest, bestModel)
...
def actualTest(boost_xDf, boost_yDf, xTest, yTest, model):
    ...
    # find the R values
    r = np.corrcoef(yTest['Yield (tons/acre)'], pred)[0,1]
...
#     r2y = yTest.tolist()
    ...
    result = sm.OLS(yTest['Yield (tons/acre)'], r2pred).fit()

    # printing the summary table
    print(result.summary())
    print('skt mape: ' + str(round(skt_mape(yTest['Yield (tons/acre)'], pred) * 100, 3)) + '%')
    print('skt smape: ' + str(round(skt_smape(yTest['Yield (tons/acre)'], pred) * 100, 3)) + '%')
    print('skt mae: ' + str(round(skt_mae(yTest['Yield (tons/acre)'], pred), 3)))
    print('skt mse: ' + str(round(skt_mse(yTest['Yield (tons/acre)'], pred), 3)))
    print('skt rmse: ' + str(round(math.sqrt(skt_mse(yTest['Yield (tons/acre)'], pred)), 3)) + '\n\n')
